...
new_methods = '''    ElementBuilder& BorderThickness(float thickness) {
...
# Find the end of ElementBuilder class
idx = dsl.find('};\n\nstruct ChildArgument')
if idx != -1:
    dsl = dsl[:idx] + '\n' + new_methods + dsl[idx:]
    with open(path_dsl, 'w', encoding='utf-8') as f:
        f.write(dsl)
    print("Inserted new methods into ElementBuilder inside CUIDsl.h")
else:
    print("Could not find ChildArgument insertion point")
