...
Element BuildPagingControlPage() {
    // ---------- 1. 常规用法 ----------
    CUI::Widgets::Ref paging1 = CUI::Widgets::PagingControl().Shared();
        paging1.TotalPages(10);
    ...
    auto btnPrev = ElevatedButton("上一页", [paging1](UIElement*) {
                paging1.CurrentPage(paging1->GetCurrentPage() - 1);
    }).Build();
    auto btnNext = ElevatedButton("下一页", [paging1](UIElement*) {
                paging1.CurrentPage(paging1->GetCurrentPage() + 1);
    }).Build();
    auto btnJump3 = ElevatedButton("跳转到第 3 页", [paging1, status1](UIElement*) {
                paging1.CurrentPage(3);
        status1->Text = "已程序化跳转到第 3 页。";
    }).Build();
    auto btnLast = ElevatedButton("最后一页", [paging1](UIElement*) {
                paging1.CurrentPage(paging1->GetTotalPages());
    }).Build();

    // ---------- 2. 大总数分页（省略号窗口） ----------
    CUI::Widgets::Ref paging2 = CUI::Widgets::PagingControl().Shared();
        paging2.TotalPages(100);
        paging2.CurrentPage(42);
    ...
    auto btnMid = ElevatedButton("跳到第 50 页", [paging2](UIElement*) {     paging2.CurrentPage(50); }).Build();
    auto btnBegin = ElevatedButton("回到第 1 页", [paging2](UIElement*) {     paging2.CurrentPage(1); }).Build();
    auto btnEnd = ElevatedButton("跳到第 100 页", [paging2](UIElement*) {     paging2.CurrentPage(100); }).Build();

    // ---------- 3. 动态配置 ----------
    CUI::Widgets::Ref paging3 = CUI::Widgets::PagingControl().Shared();
        paging3.TotalPages(20);
        paging3.CurrentPage(5);
    ...
    pageBox->OnValueChanged().Connect([paging3](NumberBox*, float value) {
                paging3.CurrentPage(static_cast<int>(value + 0.5f));
    });

    auto btnTotal10 = ElevatedButton("总页数 10", [paging3](UIElement*) {     paging3.TotalPages(10); }).Build();
    auto btnTotal50 = ElevatedButton("总页数 50", [paging3](UIElement*) {     paging3.TotalPages(50); }).Build();
    auto btnTotal200 = ElevatedButton("总页数 200", [paging3](UIElement*) {     paging3.TotalPages(200); }).Build();
    ...
    spec.source = R"(
// 1) 创建分页条
CUI::Widgets::Ref paging = CUI::Widgets::PagingControl().Shared();
