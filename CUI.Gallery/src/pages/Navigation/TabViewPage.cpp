...
Element BuildTabViewPage() {
    // ---------- 1. 常规用法 ----------
    CUI::Widgets::Ref tabView =std::make_shared<TabView>();
        tabView.Width(560.0f);
        tabView.Height(300.0f);
    ...
    CUI::Widgets::Ref status1 =MakeStatus("当前选中: [首页] (索引 0)");
    ...
    // ---------- 2. 动态管理 ----------
    CUI::Widgets::Ref dynTabs =std::make_shared<TabView>();
        dynTabs.Width(560.0f);
        dynTabs.Height(280.0f);
    dynTabs->AddTab("初始页", MakeTabPage("初始页", "通过右侧按钮动态添加 / 关闭 / 跳转标签。"), "📌", true);
        dynTabs.SelectedIndex(0);

    CUI::Widgets::Ref status2 =MakeStatus("动态添加、关闭与程序化切换标签。");
    ...
    auto btnAdd = ElevatedButton("添加标签", [dynTabs, tabCounter, status2](UIElement*) {
        ...
        dynTabs->AddTab(std::format("新标签 {}", n),
                        ...
                        "✨", true);
                dynTabs.SelectedIndex(dynTabs->GetSelectedIndex() + 1);
    ...
    auto btnWider = ElevatedButton("加宽标签 (Max 300)", [dynTabs](UIElement*) {
                dynTabs.MaxTabWidth(300.0f);
    }).Build();
    auto btnNarrow = ElevatedButton("收窄标签 (Min 60)", [dynTabs](UIElement*) {
                dynTabs.MinTabWidth(60.0f);
    }).Build();

    // ---------- 3. 固定标签（不可关闭） ----------
    CUI::Widgets::Ref pinned =std::make_shared<TabView>();
        pinned.Width(560.0f);
        pinned.Height(260.0f);
    ...
    pinned->AddTab("工作区", MakeTabPage("工作区", "混合固定 / 可关闭标签，适合常驻面板。"), "🖥️", true);
        pinned.SelectedIndex(0);

    CUI::Widgets::Ref status3 =MakeStatus("固定标签不渲染关闭按钮，也无法被 RemoveTab 关闭。");
    ...
    spec.source = R"(
// 1) 创建标签视图并添加标签
CUI::Widgets::Ref tabs =std::make_shared<TabView>();
