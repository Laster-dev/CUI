...
ShowcasePage BuildToastPage(const ShowcaseContext& ctx) {
    CUI::Widgets::Ref log =CreateShowcaseText("[就绪] 点击上方按钮触发 Toast 通知...", 12.0f, "#B5CEA8", false, "Consolas");
    CUI::Widgets::Ref count =CreateShowcaseText("活动通知: 0", 12.0f, "#9CDCFE", false, "Consolas");
    auto updateCount = [window = ctx.windowRef, count]() {
        auto center = ToastCenter::Ensure(window->GetRootElement().get());
                count.Text("活动通知: " + std::to_string(center ? center->GetActiveCount() : 0));
    };
