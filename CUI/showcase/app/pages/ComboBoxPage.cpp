...
ShowcasePage BuildComboBoxPage(const ShowcaseContext& ctx) {
    ...
    CUI::DSL::Borrow(target).Width(240.0f);

    auto input = TextField("自定义新主题").Width(280).Height(48).Build();
    CUI::DSL::Borrow(input).ForegroundToken(ThemeTokenId::TextPrimary);
    CUI::DSL::Borrow(input).PlaceholderColorToken(ThemeTokenId::TextMuted);
    CUI::DSL::Borrow(input).Foreground(ThemeManager::Instance().GetColor("textPrimary"));
