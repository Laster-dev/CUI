...
ShowcasePage BuildStackPanelPage(const ShowcaseContext&) {
    ...
    CUI::DSL::Borrow(target).AddChild(ElevatedButton("弹性元素 #1").Background(Rgb(0x007ACC)).Padding(14, 8, 14, 8).Build());
    CUI::DSL::Borrow(target).AddChild(ElevatedButton("弹性元素 #2").Background(Rgb(0x10B981)).Padding(14, 8, 14, 8).Build());
    CUI::DSL::Borrow(target).AddChild(ElevatedButton("弹性元素 #3").Background(Rgb(0xD13438)).Padding(14, 8, 14, 8).Build());
