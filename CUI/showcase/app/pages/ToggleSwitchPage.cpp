...
ShowcasePage BuildToggleSwitchPage(const ShowcaseContext& ctx) {
    CUI::Widgets::Ref target = CUI::Widgets::ToggleSwitch().Header("启用系统 GPU 硬件加速").IsOn(true).Shared();
    CUI::Widgets::Ref target2 = CUI::Widgets::ToggleSwitch().Header("开启暗黑极客主题").IsOn(false).Shared();
