...
CollapsePanel::CollapsePanel() {
    ...
    m_headerButton->SetProperty("pressedBackground", Value("#1F1F1F"));
    m_headerButton->SetProperty("borderThickness", Value(0.0f));
    m_headerButton->SetProperty("cornerRadius", Value(4.0f));
    m_headerButton->SetProperty("padding", Value(Thickness(10, 8, 10, 8)));
    ...
    m_contentHost->SetProperty("gap", Value(6.0f));
    m_contentHost->SetProperty("padding", Value(Thickness(10, 10, 10, 10)));
...
void CollapsePanel::SetHeader(const std::string& header) {
    m_headerText = header;
    if (m_headerButton) {
        m_headerButton->SetProperty("text", Value(m_isExpanded ? "▼ " + header : "▶ " + header));
    }
}
